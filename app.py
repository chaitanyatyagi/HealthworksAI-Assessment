...
.timer-box {
    ...
    font-weight: 500; color: #111111; letter-spacing: 1px;
}
.timer-unit { font-size: 13px; color: #aaa; font-family: 'IBM Plex Mono', monospace; }

[data-testid="stProgressBar"] > div > div {
    background-color: #111111 !important; border-radius: 2px !important;
}
[data-testid="stProgressBar"] > div {
    background-color: #e0e0e0 !important; border-radius: 2px !important; height: 4px !important;
}

.metrics-row { display: flex; gap: 14px; margin-bottom: 24px; }
.metric-card {
    flex: 1; background: #ffffff; border: 1px solid #e4e4e4;
    border-radius: 4px; padding: 16px 20px;
}
.metric-card .m-label {
    font-size: 10px; font-family: 'IBM Plex Mono', monospace;
    letter-spacing: 0.8px; text-transform: uppercase; color: #999; margin-bottom: 6px;
}
...
.file-row {
    ...
    padding: 9px 0; border-bottom: 1px solid #f2f2f2; font-size: 13px;
}
...
.file-badge {
    font-size: 10px; font-family: 'IBM Plex Mono', monospace;
    letter-spacing: 0.5px; padding: 3px 9px; border-radius: 3px; font-weight: 500; flex-shrink: 0;
}
...
[data-testid="stDataFrame"] th {
    background-color: #f4f4f4 !important; color: #555555 !important;
...
.chat-wrap {
    ...
    margin-bottom: 16px; padding-right: 6px;
}
...
.msg-avatar {
    ...
    font-size: 10px; font-weight: 600; flex-shrink: 0; font-family: 'IBM Plex Mono', monospace;
}
...
def process_single_file(file_path):
    """
    Pure data processing logic for a single PDF.
    UI updates are handled by the calling function's background thread.
    """
    try:
        # 1. Extract the specific chapter
        chapter4_text = extract_chapter_4(file_path)
        
        # 2. Break into manageable chunks for the LLM
        chunks = chunk_text(chapter4_text)
        
        results = []
        for chunk in chunks:
            # 3. LLM call (this is the "blocking" part)
            output = extract_from_chunk(chunk)
            results.append(output)
            
        # 4. Combine chunk results into a single structured object
        structured_data = aggregate_results(results)
        
        # 5. Format as a DataFrame
        filename = Path(file_path).stem
        df = to_dataframe(structured_data, filename)
        
        return df
    except Exception as e:
        # We log the error but return None so the batch can continue
...
def run_batch_pipeline(folder_path):
    ...
    def update_timer():
        while not stop_timer:
            ...
            # Using try/except here is a safety net for when the app stops
            try:
                timer_slot.markdown(
...
with col_desc:
    ...
    st.markdown(
        ...
        f' Extracts Chapter 4, chunks text, runs LLM field extraction, and saves a combined CSV.'
        f'</p>',
        unsafe_allow_html=True
    )
...
if st.session_state.pipeline_done and st.session_state.final_df is not None:
    ...
    with col_dl:
        with open(OUTPUT_DIR / "output.csv", "rb") as f:
            st.download_button("↓  Download CSV", f, file_name="output.csv", mime="text/csv")
...
from chatbot.main import get_intent_and_filters
from chatbot.main import retrieve_data
from chatbot.main import generate_final_response
...
def process_chat_query(user_input, df=None):
    ...
    Receives the user's query and optionally the extracted dataframe.
...
if st.session_state.chat_history:
    ...
    for msg in st.session_state.chat_history:
        ...
        if role == "user":
            ...
            # Renders a dataframe result inline in the chat
            html += (
                f'<div class="msg bot"><div class="msg-avatar">AI</div>'
    ...
    # Render any dataframe results below the chat bubbles
    for msg in st.session_state.chat_history:
...
# 1. CSS for the "Thinking" Animation (Place this at the top of your app)
st.markdown("""
<style>
...
# -------------------------------
# CHAT UI RENDERING (Existing loop)
# -------------------------------
# ... [Your existing history rendering code here] ...

# 3. Add the "Thinking" bubble if currently processing
...
# -------------------------------
# UPDATED CHAT INPUT + PROCESSING
# -------------------------------
# The 'disabled' parameter prevents double-sending
user_input = st.chat_input("Ask something about the extracted data…", disabled=st.session_state.processing)

if user_input:
    # LOCK the app and add user message
    st.session_state.processing = True
    ts = time.strftime("%H:%M")
    st.session_state.chat_history.append({"role": "user", "content": user_input, "time": ts})
    
    # Force a rerun to show the "Thinking" state and disable input immediately
    st.rerun()

# This part runs ONLY when the app is in 'processing' mode after the rerun
if st.session_state.processing:
    # Get the query from the last history item
    last_query = st.session_state.chat_history[-1]["content"]
    
    # Get Context
    df_context = st.session_state.final_df if st.session_state.get('pipeline_done') else None
    ...
    # UNLOCK the app and refresh to show the answer
    st.session_state.processing = False
    st.rerun()
# UPDATED CHAT INPUT + PROCESSING
# -------------------------------
# The 'disabled' parameter prevents double-sending
# -------------------------------
# CHAT INPUT ROW — with CSV upload
# -------------------------------
st.markdown("""
<style>
/* Wrapper to sit flush with chat input */
.upload-row {
    display: flex;
    align-items: center;
    background: #1a1a1a;
...
div[data-testid="stFileUploader"] { width: 32px !important; }
...
[data-testid="stFileUploaderDropzone"]:hover { border-color: #aaa !important; }
...
st.file_uploader(" ", type=["csv"], label_visibility="collapsed", key="chat_csv_upload")

st.markdown("</div>", unsafe_allow_html=True)

user_input = st.chat_input(
    "Ask something about the extracted data…",
